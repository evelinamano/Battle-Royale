	...
	if (!empty($query)) {
		...
		if (mysql_num_rows($result) == 0) {
			...
			if ($myrow["tutor"]=='0') {
				...
				<a href='". $safe_self ."?giveTutor=yes&user_id=$myrow[user_id]&$s' title='$langGiveTutor'>$langAdd</a></td>";
			} else {
				$tool_content .= "<td class=\"highlight\" align='center'>$langTutor<br>
				<a href='". $safe_self ."?removeTutor=yes&user_id=$myrow[user_id]&$s' title='$langRemoveRight'>$langRemove</a></td>";
			}
			// admin right
			if ($myrow["user_id"]!=$_SESSION["uid"]) {
				if ($myrow["statut"]=='1') {
					$tool_content .= "<td class='highlight' align='center'>$langAdministrator<br>
					<a href='". $safe_self ."?removeAdmin=yes&user_id=$myrow[user_id]&$s' title='$langRemoveRight'>$langRemove</a></td>";
				} else {
					$tool_content .= "<td valign='top' align='center' class='add_user'>
					<a href='". $safe_self ."?giveAdmin=yes&user_id=$myrow[user_id]&$s' title='$langGiveAdmin'>$langAdd</a></td>";
				}
			} else {
				if ($myrow["statut"]=='1') {
					...
					$tool_content .= "<td valign=\"top\" align='center'>
					<a href='". $safe_self ."?giveAdmin=yes&user_id=$myrow[user_id]&$s'>$langGiveAdmin</a></td>";
				}
			}
				$tool_content .= "<td valign=\"top\" align='center'>";
				$alert_uname = $myrow['prenom'] . " " . $myrow['nom'];
				$tool_content .= "<a href='". $safe_self ."?unregister=yes&user_id=$myrow[user_id]&$s' onClick=\"return confirmation('".addslashes($alert_uname)."');\">
