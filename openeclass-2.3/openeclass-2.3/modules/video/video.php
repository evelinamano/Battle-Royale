...
if (isset($_POST['submit']) or isset($_POST['edit_submit'])) {
		...
		if(isset($URL)) {
			...
			if (($file_name != "") && ($file_size <= $diskQuotaVideo )) {
					...
					if (preg_match('/\.(ade|adp|bas|bat|chm|cmd|com|cpl|crt|exe|hlp|hta|' .'inf|ins|isp|jse|lnk|mdb|mde|msc|msi|msp|mst|pcd|pif|reg|scr|sct|shs|' .'shb|url|vbe|vbs|wsc|wsf|wsh)$/', $file_name)) {
						...
						$tool_content .= "<a href=\"". $safe_self ."\">$langBack</a></p><br />";
	...
	elseif (isset($delete)) {
		...
		$tool_content .= "
<form method=\"POST\" action=\"". $safe_self ."?submit=yes\" enctype=\"multipart/form-data\" id=\"insert_form\" onsubmit=\"return checkrequired(this, 'titre');\">
...
if (isset($id)) {
   if($id != "") {
	...
	$tool_content .= "
<form method=\"POST\" action=\"". $safe_self ."?submit=yes\" enctype=\"multipart/form-data\" id=\"edit_form\" onsubmit=\"return checkrequired(this, 'titre');\">
        <table width=\"99%\" class=\"FormData\">
        <tbody>
    	<tr>
          <th class=\"left\" width=\"220\">&nbsp;</th>
  ...
  <div id=\"operations_container\">
    <ul id=\"opslist\">
	  <li><a href=\"". $safe_self ."?form_input=file\">$langAddV</a></li>
      <li><a href=\"". $safe_self ."?form_input=url\">$langAddVideoLink</a></li>
    </ul>
  </div>";
	}
	...
	while ($myrow = mysql_fetch_array($result)) {
		switch($table){
			case "video":
				if(isset($vodServer)) {
					...
					$videoURL = "'". $safe_self ."?action2=download&id=$myrow[1]'";
				}
		...
		if ($i%2) {
			...
			$tool_content .= "\n      <td align='center'><a href='". $safe_self ."?id=$myrow[0]&table_edit=$table&action2=edit'><img src='../../template/classic/img/edit.gif' border='0' title='$langModify'></img></a>&nbsp;&nbsp;<a href='". $safe_self ."?id=$myrow[0]&delete=yes&table=$table' onClick='return confirmation(\"".addslashes($myrow[2])."\");'><img src='../../template/classic/img/delete.gif' border='0' title='$langDelete'></img></a></td>\n    </tr>";
		$i++;
