...
if ($maxpage > 0) {
	...
	$nextpage = $page + 1;
	if ($prevpage >= 0) {
 		$tool_content .= "<small><a href='". $safe_self ."?page=$prevpage'>&lt;&lt; $langPreviousPage</a></small>&nbsp;";
	}
	if ($nextpage < $maxpage) { 
		$tool_content .= "<small><a href='". $safe_self ."?page=$nextpage'>$langNextPage &gt;&gt;</a></small>";
	}
}
...
while($row = mysql_fetch_array($result)) {
	...
	$tool_content .= <<<cData
        ...
        <td align="right">
          ...
          <a href="". $safe_self ."?choice=delete&amp;exerciseId=${row['id']}"  onclick="javascript:if(!confirm('${langConfirmYourChoice_temp}')) return false;"><img src="../../template/classic/img/delete.gif" alt="${langDelete_temp}" title="${langDelete_temp}" /></a>
cData;

	// if active
	if($row['active']) {
		if (isset($page)) {
			$tool_content .= "<a href=\"". $safe_self ."?choice=disable&amp;page=${page}&amp;exerciseId=".$row['id']."\">"."<img src='../../template/classic/img/visible.gif' alt='$langVisible' title='$langVisible' /></a>&nbsp;";
		} else {
			$tool_content .= "
			<a href='". $safe_self ."?choice=disable&amp;exerciseId=".$row['id']."'>"."<img src='../../template/classic/img/visible.gif' alt='$langVisible' title='$langVisible' /></a>&nbsp;";
		}
	} else { // else if not active
		if (isset($page)) {
			$tool_content .= "
			<a href='". $safe_self ."?choice=enable&amp;page=${page}&amp;exerciseId=".$row['id']."'>"."<img src='../../template/classic/img/invisible.gif' alt='$langVisible' title='$langVisible' /></a>&nbsp;";
		} else {
			$tool_content .= "
			<a href='". $safe_self ."?choice=enable&amp;exerciseId=".$row['id']."'>"."<img src='../../template/classic/img/invisible.gif' alt='$langVisible' title='$langVisible' /></a>&nbsp;";
		}
	}
	$tool_content .= "</td></tr>";
}
